#see Leshow's post on this backtest https://cryptotrader.org/backtests/NcwGpa7fqDtBhw8we
#https://cryptotrader.org/topics/408535/question-about-bot-timeouts
#Needs credits
...
class Init
  @init_context: (context) ->
    ...
    context.fee_percent = 0.5
    context.ha = new HeikinAshi()
    context.ichi_bull = new Ichimoku(8, 10, 11, 10, 10)
    context.ichi_bear = new Ichimoku(8, 10, 11, 11, 33)
    context.config_bull = new Config(
      0.1, -1.38, -0.20, 1.58, #lo/lc/so/sc
...
class Functions
  ...
  @lim_sell: (ins,retries,adj_percent) ->
    order = null
    x = 0
    price = ins.price
    while ((order==null)&&(x <= retries))
        x++
        order = sell(ins,null,price,60)
        price = price * (1 - adj_percent / 100)
            
  @lim_buy: (ins,retries,adj_percent) ->
    order = null
    x = 0
    price = ins.price
    while ((order==null)&&(x <= retries))
        x++
        order = buy(ins,null,price,60)
        price = price * (1 + adj_percent / 100)
  ...
  if is_bull or is_bear
    ...
    if tk_diff <= config.long_close and (c.chikou <= sar or rsi <= config.rsi_low or macd.histogram <= config.macd_low)
      if Functions.can_sell(instrument, context.min_btc)
        debug "sell"
        debug new Date(data.at) # Logs current timestamp
        Functions.lim_sell(instrument,5,0.2)
    ...
    if tk_diff >= config.short_close and (c.chikou >= sar or rsi >= config.rsi_high)
      if Functions.can_buy(instrument, context.min_btc, context.fee_percent)
        debug "buy"
        debug new Date(data.at) # Logs current timestamp
        Functions.lim_buy(instrument,5,0.2)
